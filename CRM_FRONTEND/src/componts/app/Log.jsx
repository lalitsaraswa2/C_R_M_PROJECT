...
function Log() {
  ...
  // Corrected spelling in the endpoint and response data structure
  useEffect(() => {
    const fetchCustomers = async () => {
      setIsLoading(true);
      try {
        const response = await axios.get('/coustmer?page=1&limit=1000'); // Corrected endpoint
        setCustomers(response.data.customers); // Assuming this is the correct structure
      } catch (error) {
